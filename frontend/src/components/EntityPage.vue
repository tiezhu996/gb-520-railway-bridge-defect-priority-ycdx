...
import { allowedTargets, formatDate, priorityLabel } from '../utils/format';
...
import PriorityDetailDialog from './priority/PriorityDetailDialog.vue';
...
const pendingReviewCount = computed(() =>
  props.config.key === 'priorityDecision' ? props.store.items.filter((item: DomainRecord) => item.status === 'review_pending').length : 0,
);

onMounted(() => void props.store.load(props.config.path));

function isPriority() { return props.config.key === 'priorityDecision'; }
function isPending(item: DomainRecord) { return isPriority() && item.status === 'review_pending'; }

function targetsFor(item: DomainRecord): readonly string[] {
  if (isPriority()) {
    // Pending decisions are handled only through the dedicated review dialog.
...
function canOpenReview(item: DomainRecord): boolean {
  return isPending(item) && canAtLeast('reviewer') && item.preparedBy !== session.value?.username;
}

function rowClassName({ row }: { row: DomainRecord }): string {
...
async function createDemo() {
  ...
  const created = await props.store.createRecord(props.config.path, {
    code: `${props.config.key.toUpperCase()}-${String(now).slice(-6)}`, name: `新增${config.label}`,
    ...
    evidence: '现场照片、量测记录与检查批次已完成核对', relatedCode: isPriority() ? 'DF-001' : 'IR-001',
  });
...
async function submitReview(item: DomainRecord, level: string, reason: string) {
  const ok = await props.store.reviewPriority(item, level, reason);
  if (ok) {
    detail.value = null;
    search.value = '';
  }
...
// Referenced inside the template via config/label without the props prefix.
...
<template>
  <main class="workspace">
    ...
    <section class="metrics">
      ...
      <MetricCard v-if="isPriority()" label="待复核" :value="pendingReviewCount" detail="缺陷变更触发的过期结论"/>
    ...
    <section class="table-shell">
      <el-table v-loading="store.loading" :data="store.items" :row-class-name="rowClassName">
        <el-table-column prop="code" label="编码" width="150"/>
        <el-table-column label="名称" min-width="220">
          <template #default="{ row }">
            <strong>{{ row.name }}</strong>
            <small>{{ row.facility }}</small>
            <el-tag v-if="isPending(row)" type="warning" size="small" effect="dark" class="pending-tag">待复核</el-tag>
            <small v-if="isPending(row)" class="pending-change">变化原因：{{ row.pendingChanged }}<template v-if="row.lastFinalLevel"> · 原定级 {{ priorityLabel(row.lastFinalLevel) }}</template></small>
          </template>
        </el-table-column>
        ...
        <el-table-column v-if="isPriority()" label="版本审计" min-width="240">
          <template #default="{ row }">
            <strong>v{{ row.version }} · {{ row.preparedBy }}</strong>
            <small>{{ latestRevision(row)?.actor }} · {{ latestRevision(row)?.requestId }}</small>
            <small>{{ latestRevision(row)?.reason || latestRevision(row)?.evidence }}</small>
          </template>
        </el-table-column>
        <el-table-column label="更新时间" width="170"><template #default="{ row }">{{ formatDate(row.updatedAt) }}</template></el-table-column>
        <el-table-column label="操作" :width="isPriority() ? 300 : 200">
          <template #default="{ row }">
            <div class="row-actions">
              <el-button v-if="isPriority()" link type="info" @click="detail = row">详情/历史</el-button>
              ...
              <el-button v-if="canOpenReview(row)" link type="warning" @click="detail = row">去复核</el-button>
              <span v-if="targetsFor(row).length === 0 && !canOpenReview(row) && !isPriority()" class="muted">无可用操作</span>
            </div>
          </template>
        </el-table-column>
      </el-table>
