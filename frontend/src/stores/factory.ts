...
export function createEntityStore(id: string) {
  return defineStore(id, {
    ...
    actions: {
      ...
      // reviewPriority closes a review_pending cycle for priority decisions.
      // expectedVersion comes from the row currently shown, so a stale form is
      // rejected by the backend instead of overwriting another reviewer.
      async reviewPriority(item: DomainRecord, level: string, reason: string): Promise<boolean> {
        this.loading = true; this.error = '';
        try {
          await request<DomainRecord>(`/priorities/${item.id}/review`, {
            method: 'POST', body: JSON.stringify({ level, reason, expectedVersion: item.version }),
          });
          await this.load('/priorities');
          return true;
        } catch (error) { this.error = error instanceof Error ? error.message : String(error); return false; }
        finally { this.loading = false; }
      },
